...
def findSilences(filename, dB, th):
  """
    returns a list:
  ...
  command = ["ffmpeg","-i",filename,
             "-af","silencedetect=n=" + str (dB) + "dB:d=" + str(th),
             "-f","null","-"]
...
def cut_silences(infile, outfile, dB, th):
  ...
  silences = findSilences (infile,dB,th)
...
def printHelp():
  print ("Usage:")
  print ("   silence_cutter.py [infile] [outfile] [optional: dB] [optional: s]")
  print ("   ")
  print ("        [dB]")
  print ("         Default: -30")
...
def main():
  ...
  if (int(dB) > 0):
    print("Unreasonably large dB; please check")
    return

  if (len(args) >= 4):
    threshold = args[3]

  cut_silences (infile, outfile, dB, threshold)
